...
The poll loop held the zone reader's timeout (`operator_tz.READER_TIMEOUT_SECONDS`) back from every cycle's budget, so every cycle polled for 5s instead of 10s. That held even when every active flight had already fired its `day_before` and nothing could spawn the reader, and with several active flights unrelated polls deferred to the next cycle. `_run_cycle` now computes which flights could spawn the reader this cycle, using the same `phase_markers.day_before_due` predicate `_process_flight` applies, with a first-seen flight counting as "could". It holds `_READER_RESERVE_SECONDS` back only from such a flight's start deadline, and only until the once-per-cycle memo has run. A cycle of fired flights keeps the full budget, and after the spawn a later first-seen flight does too. The pre-pass also applies `_due_for_poll`, since a flight inside its cadence interval returns before any label is asked for. A record the pre-pass cannot read, from `StateError` or `OSError`, counts as "could", so the failure still surfaces at that flight's own turn, after the flights ahead of it have polled. `tests/test_flight_assist_precheck_budget.py` pins both deadlines against the kill; three `_run_cycle` tests pin the full budget, the reserved deadline, and the reserve dropping after the spawn.

`GoogleCalendarClient._request` returned whatever `json.loads` produced. A 2xx whose body was not UTF-8 or not JSON raised a raw decode error, and a JSON array made `find_events` call `page.get` on a list and raise `AttributeError`. Neither reached the `GoogleCalendarError` handlers callers have, so `scripts/day-before-calendar.py` printed a traceback instead of `{"error": "calendar"}`. The client, which owns that contract, now raises `GoogleCalendarError` (`status_code` None) for any 2xx body that is not a UTF-8 JSON object, and for an empty body on anything but a 204 (an empty 200 used to read as "no events"), and the script's own decode catches are gone. `state._read_json_with_version` likewise reads explicitly as UTF-8, never the process locale, and wraps a non-UTF-8 file as `StateError`, the "present but unusable" contract its callers already handle; the writer states UTF-8 too, a no-op for its ASCII JSON. `scripts/reconcile.py` is unchanged: its fetch path lets an unexpected error propagate as a non-zero exit by design, and a malformed body now surfaces there as a `GoogleCalendarError` naming the gateway instead of an `AttributeError`.

Also folded in from Copilot on #311: the `_run_sweep` seam test points `DRIVE_PLANNER_STATE_DIR` at its `tmp_path`. The sweep's lodging side prunes the drive-decision store on every run, so the test could otherwise read or rewrite deployed state.
...
The same notification's calendar heads-up framed a declined football practice as a conflict and read its times off a stale `+02:00` offset. Both are fixed predicates, so they moved out of the compose prose into a script (policy review on #307): `scripts/day-before-calendar.py <flight_id>` fetches the primary calendar across the flight's conflict window (`day_before_calendar.CONFLICT_MARGIN` either side of the effective departure and arrival, the instants the boarding block uses), drops events the operator declined or that were cancelled, and renders every time on the operator's clock through the same core reader, keeping each event's own offset only when no zone is available. The `day_before` row now summarizes that list and makes no filtering or timezone call of its own; every failure comes back as a named `error` (`no_state`, `state`, `gateway`, `tier`, `calendar`) with exit 1, and the check goes out without the calendar part. Nothing retries a `day_before` that already fired, so each fixable error names on stderr the exact command to rerun for that flight. `day_label()` also names an operator zone that does not resolve on stderr instead of falling back silently. Review rounds tightened three edges. An all-day event's `date` is read as a day in the operator's zone, never at UTC midnight, which had dropped a Sep 10 all-day event from a Sep 10 evening window in Chicago. A malformed persisted departure or arrival returns `{"error": "state"}` instead of a traceback. The payload's `tz` names only a zone that actually resolved. And the precheck asks `phase_markers.day_before_due` before spawning the zone reader, so a first-seen flight more than a day out never pays for it. `travel-core/operator_tz.py` picked up the advisories deferred here from #305. Every unavailable message now says what to check. A reader killed at its timeout still has its partial stderr relayed. A payload whose zone does not resolve, whose `local_now` is naive, or whose `local_date` is not an ISO date reads as unavailable. The stale-offset artifact itself is the drive-engine half of #301.

## 0.2.136 — 2026-09-11

### drive-engine — no drive to a meeting at the anchor; meeting drives render in the operator's zone (#301)

One "Added drives" notice on 2026-09-11 carried two defects. The first: an ExamOne appointment whose `location` was byte-for-byte the operator's `current_home` produced two `Drive: ExamOne Appointment` blocks, home→home, each a degenerate one-minute event. The issue traced it to `suppression.is_trivial_leg` requiring a flight presence block the meeting could never have; that is not where it went wrong. The meeting path (`meeting_source.meeting_desired_blocks`) never reaches trivial-leg suppression at all — that rung belongs to the airport legs in `engine.py`. What was missing was the question nobody asked: is the venue the anchor itself? `scan._classify` now filters a meeting whose normalized location equals the resolved anchor (home off-trip, the current lodging on-trip) as `filtered` with reason "meeting at the anchor — no drive" — the same case-insensitive, whitespace-normalized `_same_venue` equality the neighbour rule already uses, never a geocode. The same test keeps such a meeting out of the neighbour pass: a home appointment 30 minutes after a real meeting used to count as a tight-gap neighbour, dropping the real meeting's return leg and turning the drive home into a "bridge" to the house; the real meeting now keeps its own return, correctly labelled. `meeting_desired_blocks` adds a belt to that: a leg whose origin is its destination after the lodging rewrite (a venue that is the trip's hotel) is skipped with a diagnostic, never routed.

The second: the source events carried a `dateTime` offset of `+02:00` with `timeZone: America/Chicago` — a stale travel-timezone artifact after the Amsterdam trip. `scan._extract_timezone` did what #284 taught it: a declared name contradicting its own offset is untrusted, derive the zone from the offset. That kept the INSTANT right and put the block on the wrong clock — written in `Etc/GMT-2`, announced as "15:00" when the operator's calendar showed 08:00 CDT. The instant is authoritative; the display zone is now the operator's current zone, read once per sweep through `travel-core/operator_tz.py` from the core `current-tz` reader, and `meeting_desired_blocks` writes every meeting block in it (`display_tz`). The notice's "at HH:MM" comes from the same field, so block and notice agree with what Google Calendar renders for the operator right now. An unavailable zone keeps each meeting's own — #284's offset fallback still stands underneath. Existing blocks converge too (#309, folded in from Copilot's review): the block reader keeps each event's own `start.timeZone`, and reconcile re-patches a meeting block whose zone differs from the operator's. The comparison fires only for a zone the operator reader supplied (`DesiredBlock.zone_from_operator`), so a sweep that fell back to the meeting's own zone while the reader was unavailable never flaps blocks back. A zone-only patch leaves the drive time untouched, so it goes out silently. Airport and lodging legs are untouched; they already render in the airport's zone. #300's secondary defect (calendar heads-up times parsed off a `+02:00` offset labelled `America/Chicago`) is the same artifact class in flight-assist and is addressed there.
...
#54 taught `is_real_boarding` to reject byAir's early `computed_status: boarding` flip when `computed_status_detail` was still a future-tense countdown ("Boarding starts in 1h 19min"). byAir also flips the detail itself to the present tense up to about an hour early: the live snapshot (flight 7408393, polled 06:22Z) read `boarding` / `"Boarding now"` / `computed_phase_progress 0.0546` / `dep_time null`. Nothing in the future-tense guard matches "Boarding now", so the label was relayed.

The two extra signals in that snapshot were considered and rejected. `computed_phase_progress` at 0.05 an hour out means the number tracks byAir's own ~60-minute "boarding" phase, not the airline's boarding; genuine boarding at dep−30 would read about 0.5, and no threshold separates the two cases. `dep_time` being null before departure is byAir's normal pre-OOOI state, not evidence either way.

The gate is the window flight-assist already computes: `boarding_window_open` (`phase_markers.py`) is `scheduled_dep − boarding_lead`, the same lead `boarding_lead.py` resolves for the boarding block, so the alert and the block agree by construction. `is_real_boarding` takes that window and the instant the snapshot was polled, and a "boarding" snapshot polled before the window is premature and reads as not boarding. `detect_wake_events` judges each side of the transition at its own poll instant — the fresh snapshot at `now`, the prior one at its `last_polled_at` — so a prior "Boarding now" polled before the window counts as not boarding and the first in-window poll fires `boarding_started` even though the raw label never changed. The gate is opt-in per call (window plus instant); the precheck passes both, callers without a lead keep the #54 predicate alone. `is_boarding_or_gone` carries the same pair, so a premature label no longer suppresses the gate/terminal readout (#103) or the leave-by alert (#102) either; `check_time_to_leave` gains `boarding_lead_minutes` for that. The window is anchored on the effective departure — byAir's live `dep_time` when the snapshot carries one, else the scheduled time — the same preference `calendar_reconcile._effective_times` gives the boarding block, so a delayed flight moves block and gate together and the early flip on a delayed flight is still held to the block's own start (Copilot on #306). The transition judges each snapshot against its own window, the prior one computed from its own effective departure and lead: a departure advanced between polls would otherwise read the pre-advance snapshot as already boarding and swallow the alert.
