...
- If a meeting carries `unplannable` legs, add exactly ONE line for that meeting (a meeting can have several — e.g. both the outbound and return are too far; they share one cause). Pick the leg with the largest `drive_minutes`, ties broken by list order, and use its `reason` verbatim: "No drive block for `<summary>` — `<reason>`. Reply `skip <meeting_id>` to stop seeing it." Don't add your own cause (the `reason` already says whether it's too far or doesn't fit the gap), and don't emit a line per leg.
