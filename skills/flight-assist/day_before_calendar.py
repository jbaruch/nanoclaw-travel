...
def _operator_zone(operator_tz: str | None) -> ZoneInfo | None:
    """The operator's zone as a `ZoneInfo`, or None with a stderr line when it
    cannot be resolved (the caller then keeps each event's own offset)."""
    if not operator_tz:
        return None
    try:
...
def _bounds(event: dict) -> tuple[datetime, datetime, bool] | None:
    """`(start, end, all_day)` for an event, or None when its times do not parse.

    An all-day event spans its `date` to its exclusive end `date`, both taken
    at UTC midnight — coarse, but an all-day block is a whole-day fact, not a
    clock time.
    """
    ...
    if "date" in start and "dateTime" not in start:
        ...
        return (
            ...
            datetime(after.year, after.month, after.day, tzinfo=timezone.utc),
            True,
        )
...
def calendar_conflicts(
    ...
    *,
    ...
    clock (`operator_tz`), or on the event's own offset when no zone resolves;
    `display` is the human form (`Thu Sep 10, 18:00–20:10`, or the date for an
    all-day event). An event whose times do not parse is left out — there is
    no instant to place it by.
    """
    window_start, window_end = window
    zone = _operator_zone(operator_tz)
