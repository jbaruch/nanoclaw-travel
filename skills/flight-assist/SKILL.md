...
| `delay` | "Flight `<code>` delayed by `<delay_minutes>` min. New departure: `<new_dep_time>` (local)." Negative `delay_minutes` = advanced; phrase as "moved earlier by N min". A `schedule_slip: true` event is the same surface (a delay already present on the first poll, measured against the scheduled departure) — phrase it identically |
