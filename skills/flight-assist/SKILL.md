...
It prints single-line JSON: `{"status": "...", "planned": N, "executed": N, "archived": N, "failed": [...], "airport_drive": {...}}`. `status: "ok"` means a cycle ran. `no_calendar` (no flight calendar resolved from config), `no_flights` (nothing tracked), or `{"status": "error", "error": "credentials"}` (Composio not configured — see Step 1) all mean reconciliation is inactive this cycle. The `airport_drive` object is the same shape for the drive-block reconcile and is independently `{"status": "ok", ...}` even when the top-level `status` is `no_calendar` (drive blocks live on the primary calendar); it stays a dormant zero-op summary without a Maps key / byAir URL / tracked flights. Treat any of those, and a non-empty `failed` list (those ops retry next cycle), as expected: proceed to the notification below. Reconcile output is calendar bookkeeping, never a user-facing message — do not surface it; proceed silently regardless of its result.
