...
def _desired(identity, kind="airport_departure"):
    a = datetime(2026, 7, 12, 8, tzinfo=UTC)
    return DesiredBlock(
        identity=identity,
        kind=kind,
...
def test_render_lists_each_action():
    plan = ReconcilePlan(
        creates=(Create(_desired("STN-CPH-20260712T0900Z")),),
        deletes=(Delete("cph1", "legacy orphan: no desired leg"),),
        converts=(Convert(_desired("BNA-STN-20260712T0600Z"), ("stn1", "stn2")),),
    )
    out = render_plan(plan)
    assert "+ CREATE airport_departure STN-CPH-20260712T0900Z" in out
    assert "- DELETE event cph1  (legacy orphan: no desired leg)" in out
    assert "⇄ CONVERT airport_departure BNA-STN-20260712T0600Z" in out
